...
<ul>
  <li>Ability to infer, via each Resource definition, exactly which controller actions is defined by the Controller.</li>
...
map.resources :articles, :formatted => false do |article|
  ...
  article.resources :photos, :except => :destroy
end
</code></pre>
